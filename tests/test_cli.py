...
from tests.conftest import dummy_dataset, bucket
from tests.test_fairly import create_manifest_from_template, ROOT_DIR

runner = CliRunner()

# load environment variable token ii
FAIRLY_ZENODO_TOKEN = os.environ.get("FAIRLY_ZENODO_TOKEN")

# Here we define a fixed dataset that will be used by the tests bellow
CLIENT = fairly.client(id="zenodo", token=FAIRLY_ZENODO_TOKEN)
...
def dataset_clone_commands(dummy_dataset, templates, bucket):
    ...
    remote_dataset = local_dataset.upload(CLIENT)
    ...
    ['dataset', 'clone', '--url', 'https://zenodo.org/record/7526539', '--token', FAIRLY_ZENODO_TOKEN, f'{bucket.strpath}/{remote_dataset.id}'],
